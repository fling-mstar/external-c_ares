...
DhcpNameServer
  ...
  // TODO(dpanpong): system_property_get fails when this is used from
  // Chrome (Chrome will execv(curl) in order to upload a crash report).
  // This is because Chrome deliberately destroys its connections to the
  // sockets used for getprop / setprop as a security measure.
  // Fall back to the standard /etc/resolv.conf method to get the
  // DNS server, instead.  This will need to be revisited if
  // we remove support for /etc/resolv.conf in the future.  That will
  // not happen before we switch to bionic, however, since the standard
  // libraries require /etc/resolv.conf for gethostbyname(), etc.
#elif defined(ANDROID) && !defined(BUILD_GOOGLETV)
